...
//  Pods
//
//  Created by Maxim Zakopaylov on 08/19/2020
//


import UIKit

class SCP_SCMZFlatBar: UIView {
    ...
    func set_view(_ page: Int, current: Int, current_color: UIColor) {
        ...
        let f_all_width: CGFloat = CGFloat(numberOfPage*32)
    ...
    func scroll_did(_ scrollView: UIScrollView) {
        let f_move_x: CGFloat = (32*(scrollView.contentOffset.x/scrollView.frame.size.width))
    ...
    func set_rotateDevice(_ frame: CGRect) {
        ...
        let f_width: CGFloat = CGFloat(numberOfPage*32) - 7.5
        ...
        for subview in self.subviews {
            if subview.isKind(of: UIImageView.classForCoder()) {
                if subview.tag != 5 {
                    ...
                    f_x += subview.frame.size.width + 8
                }
            }
        }
